...
if __name__ == '__main__':
    ...
    for i in range(len(opt.in_files)):
        file = opt.in_files[i]
        out_file = os.path.join(opt.out_dir, file.split(
            "\\")[-1][:file.rfind('.')] + "_text")
        out = open(out_file, "w")
        print("Processing file " + str(i + 1) +
              " out of " + str(len(opt.in_files)) + " files")
        part = 1
            ...
            while not_empty and (not opt.part or part <= opt.part):
                ...
                for sent in tqdm(conll, total=len(conll)):
                    try:
                        ...
                        for x in sent:
                            print(x.id, x.form, x.lemma,
                                  x.upos, x.head, x.deprel)
                        exit()
                del conll
                out.write(out_text)
                del out_text
